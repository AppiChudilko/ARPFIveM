...
namespace Server.Managers
{
    public class Inventory : BaseScript
    {
        ...
        */
        ...
        public Inventory()
        {
            EventHandlers.Add("ARP:Inventory:AddItem", new Action<Player, int, int, int, int, int, int, int, int>(AddItem));
            EventHandlers.Add("ARP:Inventory:AddItemPos", new Action<Player, int, float, float, float, float, float, float, int, int, int>(AddItemPos));
            EventHandlers.Add("ARP:Inventory:UpdateItem", new Action<Player, int, int, int, int, int, int, int, int>(UpdateItem));
            EventHandlers.Add("ARP:Inventory:UpdateItemOwner", new Action<Player, int, int, int>(UpdateItemOwner));
            EventHandlers.Add("ARP:Inventory:UpdateItemCount", new Action<Player, int, int>(UpdateItemCount));
            EventHandlers.Add("ARP:Inventory:UpdateItemPos", new Action<Player, int, int, float, float, float, float, float, float, int, int>(UpdateItemPos));
            EventHandlers.Add("ARP:Inventory:DeleteItem", new Action<Player, int>(DeleteItem));
            EventHandlers.Add("ARP:Inventory:GetInfoItem", new Action<Player, int>(GetInfoItem));
            EventHandlers.Add("ARP:Inventory:GetItemList", new Action<Player, int, int>(GetItemList));
            EventHandlers.Add("ARP:Inventory:GetItemListInWorld", new Action<Player, float, float>(GetItemListInWorld));
            EventHandlers.Add("ARP:Inventory:UpdateAmount", new Action<Player, int, int>(UpdateAmount));
        ...
        public static readonly dynamic[,] ItemList =
        {
            ...
            { "Рация", "", true, -1964402432, 250, 170 }, // 47
        ...
        public static void AddItem([FromSource] Player player, int itemId, int count, int ownerType, int ownerId, int countItems, int prefix, int number, int keyId)
        {
            string license = player.Identifiers["license"];
            string guid = GetPlayerGuid(player.Handle);
            Main.SaveLog("ITEM_LOG", $"ADD:{license}|{guid}|{itemId}|{ownerType}|{ownerId}|{countItems}");
            
            for (int i = 0; i < count; i++)
            {
                Appi.MySql.ExecuteQuery("INSERT INTO items (item_id, owner_type, owner_id, count, prefix, number, key_id, timestamp_update) " +
                                        $"VALUES ('{itemId}', '{ownerType}', '{ownerId}', '{countItems}', '{prefix}', '{number}', '{keyId}', '{Main.GetTimeStamp()}')");
            }
        }

        public static void AddItemPos([FromSource] Player player, int itemId, float posX, float posY, float posZ, float rotX, float rotY, float rotZ, int count, int ownerType, int ownerId)
        {
            string license = player.Identifiers["license"];
            string guid = GetPlayerGuid(player.Handle);
            Main.SaveLog("ITEM_LOG", $"ADDPOS:{license}|{guid}|{itemId}|{posX}|{posY}|{posZ}|{ownerType}|{ownerId}");
            
            for (int i = 0; i < count; i++)
            {
                Appi.MySql.ExecuteQuery("INSERT INTO items (item_id, owner_type, owner_id, pos_x, pos_y, pos_z, rot_x, rot_y, rot_z, timestamp_update) " +
                                        $"VALUES ('{itemId}', '{ownerType}', '{ownerId}', '{posZ}', '{posY}', '{posZ}', '{rotX}', '{rotY}', '{rotZ}', '{Main.GetTimeStamp()}')");
            }
        }

        public static void UpdateItem([FromSource] Player player, int id, int itemId, int ownerType, int ownerId, int countItems, int prefix, int number, int keyId)
        {
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '{itemId}', owner_type = '{ownerType}', owner_id = '{ownerId}', count = '{countItems}', prefix = '{prefix}', number = '{number}', key_id = '{keyId}', timestamp_update = '{Main.GetTimeStamp()}' where id = '{id}'");
        }

        public static void UpdateItemOwner([FromSource] Player player, int id, int ownerType, int ownerId)
        {
            Appi.MySql.ExecuteQuery($"UPDATE items SET owner_type = '{ownerType}', owner_id = '{ownerId}', timestamp_update = '{Main.GetTimeStamp()}' where id = '{id}'");
        }

        public static void UpdateItemCount([FromSource] Player player, int id, int count)
        {
            Appi.MySql.ExecuteQuery($"UPDATE items SET count = '{count}', timestamp_update = '{Main.GetTimeStamp()}' where id = '{id}'");
        }

        public static void CookFood([FromSource] Player player, int id)
        {
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '232', timestamp_update = '{Main.GetTimeStamp()}' where owner_id = '{id}' AND owner_type = 1 AND item_id = 223");
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '233', timestamp_update = '{Main.GetTimeStamp()}' where owner_id = '{id}' AND owner_type = 1 AND item_id = 224");
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '234', timestamp_update = '{Main.GetTimeStamp()}' where owner_id = '{id}' AND owner_type = 1 AND item_id = 225");
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '235', timestamp_update = '{Main.GetTimeStamp()}' where owner_id = '{id}' AND owner_type = 1 AND item_id = 226");
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '236', timestamp_update = '{Main.GetTimeStamp()}' where owner_id = '{id}' AND owner_type = 1 AND item_id = 227");
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '237', timestamp_update = '{Main.GetTimeStamp()}' where owner_id = '{id}' AND owner_type = 1 AND item_id = 228");
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '238', timestamp_update = '{Main.GetTimeStamp()}' where owner_id = '{id}' AND owner_type = 1 AND item_id = 229");
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '239', timestamp_update = '{Main.GetTimeStamp()}' where owner_id = '{id}' AND owner_type = 1 AND item_id = 230");
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '240', timestamp_update = '{Main.GetTimeStamp()}' where owner_id = '{id}' AND owner_type = 1 AND item_id = 231");
            
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '246', timestamp_update = '{Main.GetTimeStamp()}' where owner_id = '{id}' AND owner_type = 1 AND item_id = 241");
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '247', timestamp_update = '{Main.GetTimeStamp()}' where owner_id = '{id}' AND owner_type = 1 AND item_id = 242");
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '248', timestamp_update = '{Main.GetTimeStamp()}' where owner_id = '{id}' AND owner_type = 1 AND item_id = 243");
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '249', timestamp_update = '{Main.GetTimeStamp()}' where owner_id = '{id}' AND owner_type = 1 AND item_id = 244");
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '250', timestamp_update = '{Main.GetTimeStamp()}' where owner_id = '{id}' AND owner_type = 1 AND item_id = 245");
        }

        public static void SellMeat([FromSource] Player player, int id, int shopId)
        {
            int money = 0;
            int count = 0;
            
            foreach (DataRow row in Appi.MySql.ExecuteQueryWithResult($"SELECT * FROM items WHERE owner_id = {id} AND owner_type = 1").Rows)
            {
                if ((int) row["item_id"] == 223)
                {
                    money += 130;
                    count++;
                    DeleteItem(player, (int) row["id"]);
                }
                else if ((int) row["item_id"] == 224)
                {
                    money += 130;
                    count++;
                    DeleteItem(player, (int) row["id"]);
                }
                else if ((int) row["item_id"] == 225)
                {
                    money += 90;
                    count++;
                    DeleteItem(player, (int) row["id"]);
                }
                else if ((int) row["item_id"] == 226)
                {
                    money += 100;
                    count++;
                    DeleteItem(player, (int) row["id"]);
                }
                else if ((int) row["item_id"] == 227)
                {
                    money += 190;
                    count++;
                    DeleteItem(player, (int) row["id"]);
                }
                else if ((int) row["item_id"] == 228)
                {
                    money += 5;
                    count++;
                    DeleteItem(player, (int) row["id"]);
                }
                else if ((int) row["item_id"] == 229)
                {
                    money += 80;
                    count++;
                    DeleteItem(player, (int) row["id"]);
                }
                else if ((int) row["item_id"] == 230)
                {
                    money += 40;
                    count++;
                    DeleteItem(player, (int) row["id"]);
                }
                else if ((int) row["item_id"] == 231)
                {
                    money += 1;
                    count++;
                    DeleteItem(player, (int) row["id"]);
            ...
            Business.AddMoney(shopId, Convert.ToInt32(money * 0.1));
            player.TriggerEvent("ARP:SellMeat", Convert.ToInt32(money / 2), count);
        }

        public static void SellFish([FromSource] Player player, int id, int shopId)
        {
            int money = 0;
            int count = 0;
            ...
            foreach (DataRow row in Appi.MySql.ExecuteQueryWithResult($"SELECT * FROM items WHERE owner_id = {id} AND owner_type = 1").Rows)
            {
                if ((int) row["item_id"] == 241)
                {
                    money += 60;
                    count++;
                    DeleteItem(player, (int) row["id"]);
                }
                else if ((int) row["item_id"] == 242)
                {
                    money += 20;
                    count++;
                    DeleteItem(player, (int) row["id"]);
                }
                else if ((int) row["item_id"] == 243)
                {
                    money += 80;
                    count++;
                    DeleteItem(player, (int) row["id"]);
                }
                else if ((int) row["item_id"] == 244)
                {
                    money += 40;
                    count++;
                    DeleteItem(player, (int) row["id"]);
                }
                else if ((int) row["item_id"] == 245)
                {
                    money += 20;
                    count++;
                    DeleteItem(player, (int) row["id"]);
            ...
            Business.AddMoney(shopId, Convert.ToInt32(money * 0.1));
            player.TriggerEvent("ARP:SellFish", money, count);
        }

        public static void UpdateItemPos([FromSource] Player player, int id, int itemId, float posX, float posY, float posZ, float rotX, float rotY, float rotZ, int ownerType, int ownerId)
        {
            string license = player.Identifiers["license"];
            string guid = GetPlayerGuid(player.Handle);
            Main.SaveLog("ITEM_LOG", $"UPDATEPOS:{license}|{guid}|{id}|{itemId}|{posX}|{posY}|{posZ}|{ownerType}|{ownerId}");
            Appi.MySql.ExecuteQuery($"UPDATE items SET item_id = '{itemId}', owner_type = '{ownerType}', owner_id = '{ownerId}', pos_x = '{posX}', pos_y = '{posY}', pos_z = '{posZ}', rot_x = '{rotX}', rot_y = '{rotY}', rot_z = '{rotZ}', timestamp_update = '{Main.GetTimeStamp()}' where id = '{id}'");
        }

        public static void DeleteItem([FromSource] Player player, int id)
        {
            Appi.MySql.ExecuteQuery($"DELETE FROM items WHERE id = '{id}'");
        }

        public static void GetInfoItem([FromSource] Player player, int id)
        {
            foreach (DataRow row in Appi.MySql.ExecuteQueryWithResult($"SELECT * FROM items WHERE id = {id}").Rows)
            {
                TriggerClientEvent(player, "ARP:Inventory:GetInfoItem", (int) row["id"], (int) row["item_id"], (int) row["owner_type"], (int) row["owner_id"], (int) row["count"], (int) row["prefix"], (int) row["number"], (int) row["key_id"]);
                return;
            }
        }

        public static void GetItemList([FromSource] Player player, int ownerId, int ownerType)
        {
            var data = new Dictionary<string, string>();
            var data2 = new Dictionary<string, int>();

            foreach (DataRow row in Appi.MySql.ExecuteQueryWithResult($"SELECT * FROM items WHERE owner_id = {ownerId} AND owner_type = {ownerType} ORDER BY item_id DESC").Rows)
            {
                if ((int) row["prefix"] > 0 && (int) row["number"] > 0)
                    data.Add(((int) row["id"]).ToString(), (int) row["prefix"] + "-" + (int) row["number"]);
                else if ((int) row["key_id"] > 0)
                {
                    if ((int) row["item_id"] >= 265 && (int) row["item_id"] <= 273)
                        data.Add(((int) row["id"]).ToString(), row["prefix"] + "|" + row["key_id"]);
                    else
                        data.Add(((int) row["id"]).ToString(), "#" + (int) row["key_id"]);
                }
                else
                    data.Add(((int) row["id"]).ToString(), "");
                
                data2.Add(((int) row["id"]).ToString(), (int) row["item_id"]);
            }
            
            User.SendToPlayerItemListMenu(player, data, data2, ownerId, ownerType);
        }

        public static void GetItemListInWorld([FromSource] Player player, float x, float y)
        {
            var data = new Dictionary<string, string>();
            var data2 = new Dictionary<string, int>();
            ...
            foreach (DataRow row in Appi.MySql
                .ExecuteQueryWithResult(
                    $"SELECT * FROM items WHERE DISTANCE(POINT(pos_x, pos_y), POINT({x}, {y})) < 2 AND owner_type = 0 ORDER BY item_id DESC")
                .Rows)
            {
                if ((int) row["prefix"] > 0 && (int) row["number"] > 0)
                    data.Add(((int) row["id"]).ToString(), (int) row["prefix"] + "-" + (int) row["number"]);
            ...
            User.SendToPlayerItemWorldListMenu(player, data, data2);
        }

        public static void UpdateAmount([FromSource] Player player, int ownerId, int ownerType)
        {
            var data = Appi.MySql.ExecuteQueryWithResult($"SELECT * FROM items WHERE owner_id = {ownerId} AND owner_type = {ownerType} ORDER BY id DESC").Rows.Cast<DataRow>().ToDictionary(row => ((int) row["id"]).ToString(), row => (int) row["item_id"]);
            User.SendToPlayerItemListUpdateAmountMenu(player, data, ownerId, ownerType);
        }
        ...
        public static void LoadAllItems(Player player)
        {
            List<InventoryWorldData> ObjectWorldList = (from DataRow row in Appi.MySql.ExecuteQueryWithResult("SELECT * FROM items WHERE owner_type = 0").Rows
                select new InventoryWorldData()
                {
                    id = (int) row["id"],
            ...
            TriggerClientEvent(player, "ARP:Inventory:AddWorldList", ObjectWorldList);
        }
    }
}
