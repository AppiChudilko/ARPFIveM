...
{
    public class Tattoo : BaseScript
    {
        public static void LoadAll()
        {
            for (int i = 0; i < Main.Tattoos.Length / 4; i++)
            {
                Vector3 shopPos = new Vector3((float) Main.Tattoos[i, 0], (float) Main.Tattoos[i, 1], (float) Main.Tattoos[i, 2]);

                var blip = World.CreateBlip(shopPos);
                blip.Sprite = (BlipSprite) 75;
                blip.Name = Lang.GetTextToPlayer("_lang_85");
        ...
        public static void CheckPosForOpenMenu()
        {
            ...
            int shopId = GetIdInRadius(playerPos, 2f);
            if (shopId == -1) return;
            switch (shopId)
            {
                case 94:
                    MenuList.ShowTattooShopMenu("shopui_title_tattoos", "shopui_title_tattoos", shopId);
                    break;
                case 95:
                    MenuList.ShowTattooShopMenu("shopui_title_tattoos2", "shopui_title_tattoos2", shopId);
                    break;
                case 96:
                    MenuList.ShowTattooShopMenu("shopui_title_tattoos3", "shopui_title_tattoos3", shopId);
                    break;
                case 97:
                    MenuList.ShowTattooShopMenu("shopui_title_tattoos4", "shopui_title_tattoos4", shopId);
                    break;
                case 98:
                    MenuList.ShowTattooShopMenu("shopui_title_tattoos5", "shopui_title_tattoos5", shopId);
                    break;
            }
        }

        public static int GetIdInRadius(Vector3 pos, float radius)
        {
            for (int i = 0; i < Main.Tattoos.Length / 4; i++)
            {
                if (Main.GetDistanceToSquared(pos, new Vector3((float) Main.Tattoos[i, 0], (float) Main.Tattoos[i, 1], (float) Main.Tattoos[i, 2])) < radius)
                    return Convert.ToInt32(Main.Tattoos[i, 3]);
            }
            return -1;
        }

        public static double[,] GetInRadius(Vector3 pos, float radius)
        {
            for (int i = 0; i < Main.Tattoos.Length / 4; i++)
            {
                if (Main.GetDistanceToSquared(pos, new Vector3((float) Main.Tattoos[i, 0], (float) Main.Tattoos[i, 1], (float) Main.Tattoos[i, 2])) < radius)
        ...
        {
            var shopPosPrew = new Vector3((float) Main.Tattoos[0, 0], (float) Main.Tattoos[0, 1], (float) Main.Tattoos[0, 2]);
            for (int i = 0; i < Main.Tattoos.Length / 4; i++)
            {
                var shopPos = new Vector3((float) Main.Tattoos[i, 0], (float) Main.Tattoos[i, 1], (float) Main.Tattoos[i, 2]);
        ...
        public static string FindTattooCollection(string name)
        {
            for (int i = 0; i < TattooList.Length / 6; i++)
            {
                if (name == (string) TattooList[i, 0])
                    return (string) TattooList[i, 1];
            }
            return "";
        }

        public static string FindTattooOverlay(string name)
        {
            for (int i = 0; i < TattooList.Length / 6; i++)
            {
                if (name == (string) TattooList[i, 0])
                    return (User.Skin.SEX == 1) ? (string) TattooList[i, 3] : (string) TattooList[i, 2];
            }
            return "";
        }
        
        //Name, Collection, OverlayMale, OverlayFemale, Zone, Price
        public static dynamic[,] TattooList =
        {
